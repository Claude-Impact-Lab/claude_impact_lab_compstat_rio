...
2. **Dinâmica criminal (qualitative):** `dados/disk_denuncia.csv` (anonymous tips) + `dados/relints/*.docx` (intelligence reports).
...
4. **Polígonos da FM:** `dados/sh_area_forca/areas_forca_municipal.*` — ESRI shapefile in **EPSG:4326 (WGS84)**.
...
- **`dados/relints/`** files are prefixed `Cópia de RI_…docx` on disk even though `README.md` lists them without the prefix — match by suffix, not exact name.
...
- Analysis/ETL code lives in `project/backend/etl/`. The shipped `dados/` directory — including `dados/relints/` and `dados/sh_area_forca/` — is read-only input. Frontend is `project/frontend/` (Vite + React). The backend FastAPI exposes the ETL via SSE at `/api/build/*`.
